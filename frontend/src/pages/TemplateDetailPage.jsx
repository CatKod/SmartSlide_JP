...
export function TemplateDetailPage({ nav, templateId, profile, setProfile }) {
  ...
  return <AppLayout nav={nav} active="templates" profile={profile} setProfile={setProfile}>
    ...
    <section className="detail-card">
      <div className="preview"><img src={t.image} alt={t.title}/></div>
      <div className="detail-info">
        <h1>{t.title}</h1>
        <p>{t.description}</p>
        <div className="meta large">
          <span><Star size={18}/> {t.rating}</span>
          <span><Download size={18}/> {t.downloads}</span>
          <span>{(t.slidesData || []).length}<Bi jp="枚" vi="trang" profile={profile}/></span>
        </div>
        <div className="chips">{(t.tags || []).map(tag=><span key={tag}>{tag}</span>)}</div>
        <div className="template-detail-actions">
          <button className="outline" onClick={() => nav('editor', { templateId: t._id, deckId: null })}><Bi jp="使用する" vi="Sử dụng" profile={profile}/></button>
          <button className="pink" onClick={() => nav('editor',{templateId: t._id, deckId:null})}><Bi jp="このテンプレートで作成" vi="Tạo bằng mẫu này" profile={profile}/></button>
        </div>
      </div>
    </section>
  </AppLayout>
